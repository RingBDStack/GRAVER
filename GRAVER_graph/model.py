...
import sys
sys.path.append("../graver-main/GRAVER_graph")
...
class downprompt(nn.Module):
    ...
    def forward(self, features, adj, gcn, idx, batch, seq, graphon_list, labels=None, train=0):
        ...
        rawret = torch_scatter.scatter(src=pretrain_embs1,index=batch,dim=0,reduce='mean')
        rawret = rawret.cuda()
        num = rawret.shape[0]
        if train == 1:
